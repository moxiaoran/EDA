...
temp_sp <- as(landsat_dc_july16, "SpatialPolygonsDataFrame")
temp_sf <- st_as_sf(temp_sp)

temp_sf <- temp_sf %>%
  rename(temp = LST_F_20160702)

water_sp <- as(water, "Spatial")
water_sf <- st_as_sf(water_sp)

ggplot(temp_sf)+
  geom_sf(aes(fill = temp)) +
  geom_sf(data = water_sf, fill = "white", color = "grey70") +
  coord_sf() +
  scale_fill_gradientn(colors = rcartocolor::carto_pal(name = "TealRose", n = 7), breaks = c(70, 83, 100), labels = c("70", "83", "100"), name = "") +
  theme_minimal()
...
try to use tigris package to add river info on the map


```{r}
dc_state <- counties(state = "DC")
